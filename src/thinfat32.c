	...
	while(count > 0) {
		...
		while(i > 0) {  // really suboptimal for performance.  optimize.
            ...
            */
            ...
            //       fp->currentByte, tracking, (i<512 ? i:512), fp->size, fp->pos);
            //tf_printHex(tf_info.buffer, 512);
            
			memcpy( &tf_info.buffer[ tracking ], src, (i<512 ? i:512));
			tf_info.sectorFlags |= TF_FLAG_DIRTY; // Mark this sector as dirty
            
            if (fp->pos + (i<512 ? i:512) > fp->size)
            {
            //    tf_printf("\r\n++ increasing filesize:  %x + %x > %x",
            //           fp->pos , (i<512 ? i:512) , fp->size);
                fp->size += (i<512 ? i:512) - (fp->pos % 512);
            }
            ...
            if(tf_unsafe_fseek(fp, 0, fp->pos + (i<512 ? i:512))) {
				return -1;
			}
			i -= (i<512 ? i:512);
            //*/
